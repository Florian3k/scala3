...
import scala.tools.asm.tree.analysis.{Value, Frame, Analyzer}
...
object BytecodeUtils {
  ...
  def hasCallerSensitiveAnnotation(methodNode: MethodNode): Boolean =
    ...
    methodNode.visibleAnnotations.nn.stream.nn.filter(ann =>
      ann.nn.desc == "Lsun/reflect/CallerSensitive;" || ann.nn.desc == "Ljdk/internal/reflect/CallerSensitive;"
    ).nn.findFirst.isPresent
  ...
  @tailrec def previousLineNumber(insn: AbstractInsnNode): Option[Int] = {
    ...
    prev match {
      ...
      case _ => previousLineNumber(prev.nn)
    }
  }
  ...
  @tailrec def nextExecutableInstruction(insn: AbstractInsnNode, alsoKeep: AbstractInsnNode => Boolean = Set()): Option[AbstractInsnNode] = {
    ...
    if (next == null || isExecutable(next) || alsoKeep(next)) Option(next.asInstanceOf[AbstractInsnNode])
  ...
  @tailrec def nextExecutableInstructionOrLabel(insn: AbstractInsnNode): Option[AbstractInsnNode] = {
    val next = insn.getNext
    if (next == null || isExecutable(next) || next.isInstanceOf[LabelNode]) Option(next.asInstanceOf[AbstractInsnNode])
    ...
    @tailrec def find(insn: AbstractInsnNode | Null): Option[MethodInsnNode] = {
      if (insn == null) None
        ...
        case mi: MethodInsnNode =>
          if (such(mi) && noMoreInvoke(insn.nn.getNext)) Some(mi)
          else None
        case _ =>
          find(insn.nn.getNext)
      }
    }
    find(method.instructions.nn.getFirst)
  }
  ...
  def sameTargetExecutableInstruction(a: JumpInsnNode, b: JumpInsnNode): Boolean = {
    ...
    //   LabelNode(b)
    //   Instr
    nextExecutableInstruction(a.label.nn) == nextExecutableInstruction(b.label.nn)
  }

  def removeJumpAndAdjustStack(method: MethodNode, jump: JumpInsnNode): Unit = {
    val instructions = method.instructions.nn
    ...
    if ((op >= IFEQ && op <= IFLE) || op == IFNULL || op == IFNONNULL) {
      ...
      assert(jump.getOpcode == GOTO, s"Cannot remove JSR instruction in ${method.name} (at ${method.instructions.nn.indexOf(jump)}")
    }
    instructions.remove(jump)
  }

  def finalJumpTarget(source: JumpInsnNode): LabelNode | Null = {
    @tailrec def followGoto(label: LabelNode, seenLabels: Set[LabelNode]): LabelNode | Null = nextExecutableInstruction(label) match {
      case Some(Goto(dest)) =>
  ...
  def substituteLabel(reference: AnyRef, from: LabelNode, to: LabelNode): Unit = {
    ...
    (reference: @unchecked) match {
      ...
      case switch: LookupSwitchInsnNode => substList(switch.labels.nn); if (switch.dflt == from) switch.dflt = to
      case switch: TableSwitchInsnNode  => substList(switch.labels.nn); if (switch.dflt == from) switch.dflt = to
  ...
  def callsiteTooLargeAfterInlining(caller: MethodNode, callee: MethodNode): Boolean = {
    ...
    def roughUpperBound(methodNode: MethodNode): Int = methodNode.instructions.nn.size * 8
  ...
  def cloneLabels(methodNode: MethodNode): Map[LabelNode, LabelNode] = {
    methodNode.instructions.nn.iterator.asScala.collect({
  ...
  def cloneLocalVariableNodes(methodNode: MethodNode, labelMap: Map[LabelNode, LabelNode], calleeMethodName: String, localIndexMap: Int => Int): List[LocalVariableNode] = {
    ...
    for (localVariable <- methodNode.localVariables.nn.iterator.nn.asScala) {
  ...
  def cloneTryCatchBlockNodes(methodNode: MethodNode, labelMap: Map[LabelNode, LabelNode]): List[TryCatchBlockNode] = {
    methodNode.tryCatchBlocks.nn.iterator.nn.asScala.map(tryCatch => new TryCatchBlockNode(
  ...
  implicit class FrameExtensions[V <: Value](val frame: Frame[V]) extends AnyVal {
     ...
     */
    def peekStack(n: Int): V | Null = frame.getStack(frame.getStackSize - 1 - n)
