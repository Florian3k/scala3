...
trait ClassLikeSupport:
  ...
  def mkClass(classDef: ClassDef)(
    ...
    val supertypes = getSupertypes(using qctx)(classDef)
      ...
      .map {
        case (symbol, tpe) =>
          val signature = signatureWithName(tpe.asSignature(classDef, Symbol.noSymbol))
    ...
    val selfSignature: DSignature = signatureWithName(typeForClass(classDef).asSignature(classDef, Symbol.noSymbol))
  ...
  extension (c: ClassDef)
    ...
    def getParentsAsLinkToTypes: List[LinkToType] =
      c.getParentsAsTreeSymbolTuples.map {
        (tree, symbol) => LinkToType(tree.asSignature(c, c.symbol), symbol.dri, bareClasslikeKind(symbol))
      }
  ...
  def parseMethod(
    ...
    println(s"    ${c.symbol.name} -> ${methodSymbol.name}")
